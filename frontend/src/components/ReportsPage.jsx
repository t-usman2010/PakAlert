    ...
    >
      <div className='max-w-6xl mx-auto px-4 sm:px-6 lg:px-8'>
        <div className='text-center mb-12'>
          <div
            className={`inline-flex items-center justify-center w-20 h-20 rounded-3xl mb-6 transition-all duration-300 ${
              isLight
          ...
          >
            <MessageSquare className={isLight ? 'text-blue-600' : 'text-blue-400'} size={36} />
          </div>
          <h1
            className={`text-4xl font-bold mb-4 bg-gradient-to-r bg-clip-text text-transparent ${
              isLight ? 'from-gray-800 to-blue-700' : 'from-white to-blue-300'
            }`}
          >
            Community Weather Reports
          </h1>
          <p className={`text-lg max-w-2xl mx-auto leading-relaxed ${isLight ? 'text-gray-600' : 'text-slate-400'}`}>
        ...
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-8'>
          <div
            className={`rounded-3xl shadow-2xl border p-8 transition-all duration-300 ${
              isLight
          ...
          >
            ...
            <form onSubmit={handleSubmit} className='space-y-6'>
              ...
              <div className='space-y-3'>
                ...
                <div className='relative'>
                  ...
                  <FileText
                    size={20}
                    className={`absolute left-4 top-4 transition-colors ${isLight ? 'text-gray-400' : 'text-slate-400'}`}
                  />
                </div>
              </div>
            ...
            {status && (
              <div
                className={`mt-6 p-5 rounded-2xl border-2 flex items-start transition-all duration-300 ${
                  statusType === 'success'
                    ? isLight
              ...
              >
                {statusType === 'success' ? (
                  <CheckCircle2
                    size={24}
                    className={`mr-4 mt-0.5 flex-shrink-0 ${isLight ? 'text-blue-600' : 'text-blue-400'}`}
                  />
                ) : (
                  <AlertCircle
                    size={24}
                    className={`mr-4 mt-0.5 flex-shrink-0 ${isLight ? 'text-red-600' : 'text-red-400'}`}
                  />
                )}
                <div>
                  <p
          ...
          >
            <div className='flex items-center justify-between mb-8'>
              <h2 className={`text-2xl font-bold flex items-center ${isLight ? 'text-gray-800' : 'text-white'}`}>
                <div
                  className={`p-2 rounded-xl mr-3 ${
                    isLight
                      ? 'bg-blue-100 text-blue-600'
              ...
              <div className='text-center py-16'>
                <div
                  className={`inline-flex items-center justify-center w-20 h-20 rounded-3xl mb-4 ${
                    isLight ? 'bg-gray-100' : 'bg-slate-700/50 backdrop-blur-lg'
                  }`}
                >
                  <Cloud className={isLight ? 'text-gray-400' : 'text-slate-500'} size={36} />
                </div>
                <p className={`font-semibold text-lg mb-2 ${isLight ? 'text-gray-700' : 'text-slate-300'}`}>
              ...
              <div className='space-y-5 max-h-[500px] overflow-y-auto pr-3'>
                {reports.map((r) => {
                  ...
                  return (
                    <div
                      key={r._id}
                      className={`border-2 rounded-2xl p-5 hover:shadow-lg transition-all duration-300 backdrop-blur-sm ${
                        isLight
                    ...
                    >
                      <div className='flex items-start justify-between mb-4'>
                        <div className='flex items-center'>
                          ...
                          >
                            <WeatherIcon className={isLight ? 'text-blue-600' : 'text-blue-400'} size={20} />
                          </div>
                          <div>
                            <p className={`font-bold text-lg ${isLight ? 'text-gray-900' : 'text-white'}`}>
                              {r.reporter}
                            </p>
                            <div className={`flex items-center text-sm mt-1 ${isLight ? 'text-gray-600' : 'text-slate-400'}`}>
                        ...
                        <div
                          className={`flex items-center text-xs font-medium px-3 py-1 rounded-full ${
                            isLight ? 'text-gray-500 bg-gray-100' : 'text-slate-400 bg-slate-600/30 backdrop-blur-sm'
                          }`}
                        >
                      ...
                      >
                        <div className={`flex items-center text-xs ${isLight ? 'text-gray-500' : 'text-slate-400'}`}>
                        ...
                        <span className={`text-xs font-medium ${isLight ? 'text-gray-400' : 'text-slate-500'}`}>
